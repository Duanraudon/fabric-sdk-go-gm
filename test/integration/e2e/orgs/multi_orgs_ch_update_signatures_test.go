...
func loadExternalSignature(t *testing.T, org, chConfigPath, user, sigDir string) *common.ConfigSignature {
	...
	if !isSDKSigning {
		...
		case *ecdsa.PrivateKey:
			newSig, e := utils.SignatureToLowS(&k.PublicKey, sig)
			require.NoError(t, e, "failed to switch ecdsa signature to LowS")
			sig = newSig
		case *sm2.PrivateKey:
			newSig, e := sw.Sm2SignatureToLowS(&k.PublicKey, sig)
			require.NoError(t, e, "failed to switch sm2 signature to LowS")
			sig = newSig
		default:
			t.Fatalf("Error private type")
		}
	}
