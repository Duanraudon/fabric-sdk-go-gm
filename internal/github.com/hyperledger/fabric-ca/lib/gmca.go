...
import (
	...
	x509GM "github.com/Duanraudon/cryptogm/x509"
...
func generate(priv crypto.Signer, req *csr.CertificateRequest) (csr []byte, err error) {
	...
	csr, err = x509GM.CreateCertificateRequest(rand.Reader, &tpl, priv)
	if err != nil {
		log.Errorf("failed to generate a sm2 CSR: %v", err)
		return
	}
	block := pem.Block{
...
// 		}
// 		puknew := &puk
// 		cert, err = x509GM.CreateCertificate(rand.Reader, template, rootca, puknew, rootSigner)
// 		break
...
// 	clientCert, err := x509GM.ParseCertificate(cert)
...
func createGmSm2Cert(key bccsp.Key, req *csr.CertificateRequest, signer crypto.Signer) (cert []byte, err error) {
	...
	//cert, err = gm.CreateCertificateToMem(template, rootca, rootSigner)
	switch sm2Template.PublicKey.(type) {
	case *sm2.PublicKey:
		pub := sm2Template.PublicKey.(*sm2.PublicKey)
		var puk = sm2.PublicKey{
			Curve: sm2.P256(),
			X:     pub.X,
			Y:     pub.Y,
		...
		puknew := &puk
		cert, err = x509GM.CreateCertificate(rand.Reader, sm2Template, sm2Template, puknew, signer)
		break
	case *ecdsa.PublicKey:
		pub := sm2Template.PublicKey.(*ecdsa.PublicKey)
		var puk = sm2.PublicKey{
			Curve: sm2.P256(),
			X:     pub.X,
			Y:     pub.Y,
