...
// ConfigSecAlgo config ctx
func ConfigSecAlgo(ctx contextApi.Client) {
	if c, ok := ctx.(*context.Client); ok {
		cryptosuite.SecAlgo = c.Providers.CryptoSuiteConfig().SecurityAlgorithm()
	}
